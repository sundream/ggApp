...
function logger.print(...)
    ...
    print(...)
    local console = skynet.getenv("console")
    if console then
...
function logger.check_loglevel(loglevel)
    if type(loglevel) == "string" then
        ...
        loglevel = assert(logger.NAME_LEVEL[loglevel],string.format("invalid loglevel: %s",loglevel))
    else
...
function logger.init()
    ...
    if not logger.service then
        -- uniqueservice: 启动一个唯一服务，如果服务该服务已经启动，则返回已启动的服务地址
