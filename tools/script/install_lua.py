...
def install(download_url,install_path):
    ...
    cmds.append("cd %s/%s && make %s && sudo make install" % (install_path,program,platform))
    result = 0
    for cmd in cmds:
        print(cmd)
...
def main():
    ...
    parser.add_option("-s","--soft_version",help=u"[optional,default=%default] 版本",default="5.3.5")
    ...
    download_url = "http://www.lua.org/ftp/lua-%s.tar.gz" % (soft_version)
