...
area = "%(area)s"           -- 大区ID
env = "%(env)s"             -- 集群环境,如dev--内网,test--外网测试，stable--外网正式
id = "%(id)s"               -- 服务器ID
name = "%(name)s"           -- 服务器名
index = %(index)s           -- 服务器编号
type = "%(type)s"           -- 服务器类型
...
def generate_gameserver_config(out,servers_config):
    ...
    for serverid,server in servers.iteritems():
        ...
        %s = {
            address = "%s:%s",
            index = %s,
        },""" % (serverid,server["cluster_ip"],server["cluster_port"],server["index"]))
...
def main():
    usage = "usage: python %prog [options]\n\te.g: python %prog --config=servers.dev.config [--out=config]"
    ...
    quite = options.quite
    new,updated = generate_gameserver_config(out,servers_config)
    if not quite:
        print("op=generate_gameserver_config,out=%s" % (out))
